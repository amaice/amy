...
public class AmySpawnerTile extends BlockEntity {
    public AmySpawnerTile(BlockPos pos, BlockState state) {
        super(TileEntityInit.AMY_SPAWNER.get(), pos, state);
    }
    int timer = 0;
    int seconds = 10;

    public static <T extends BlockEntity> void tick(Level level, BlockPos pos, BlockState state, T be) {
        AmySpawnerTile tile = (AmySpawnerTile) be;
